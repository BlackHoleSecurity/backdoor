...
class YouTubeDownloader
{
    ...
    {
        $this->link_pattern =
            "/^(?:http(?:s)?:\/\/)?(?:www\.)?(?:m\.)?(?:youtu\.be\/|youtube\.com\/(?:(?:watch)?\?(?:.*&)?v(?:i)?=|(?:embed)\/))([^\?&\"'>]+)/";
    }
    ...
    private function getVideoInfo()
    {
        return file_get_contents(
            "https://www.youtube.com/get_video_info?video_id=" .
                $this->extractVideoId($this->video_url) .
                "&cpn=CouQulsSRICzWn5E&eurl&el=adunit"
        );
    }
...
if ($_GET['get'] == "info") {
    if (isset($_POST['submit'])) {
        ...
        if (
            !empty($youtubeURL) &&
            !filter_var($youtubeURL, FILTER_VALIDATE_URL) === false
        ) {
            ...
            if ($downloader->hasVideo()) {
                ...
                $videoFileName =
                    strtolower(str_replace(' ', '_', $videoTitle)) .
                    '.' .
                    $videoFormat;
